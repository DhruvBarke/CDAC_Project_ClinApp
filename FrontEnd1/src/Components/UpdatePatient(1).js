...
function UpdatePatient(){
    ...
    const validateAddress=(e)=>{
        ...
        if(address === ""){
            setError({...Error,address_error:"Please enter Last Name"});
            setFlag({...flag,address:false});
            console.log(flag.address);

        }
        else{
            setError({...Error,address_error:""});
    ...
    useEffect(() => {
        ...
        setData({patientId:patient.patient_id,firstName:patient.firstName,lastName:patient.lastName,
            gender:patient.gender,dob:patient.dob,mobileNumber:patient.mobileNumber,
            blood_group:patient.blood_group,address:patient.address,login_id:patient.login_id})
    },[]);
        ...
        <h2 className='text-center'>Update Information </h2>
         ...
         <form method="POST">
            ...
            <div style={{ marginTop: '10px' }} className = "form-group">
                ...
                <input type="text" placeholder={data.address} name="address" className="form-control" 
                    value={data.address} onChange={changeHandler} onBlur={validateAddress}/>
                    <span className="text text-danger">{Error.address_error}</span>
            </div>
